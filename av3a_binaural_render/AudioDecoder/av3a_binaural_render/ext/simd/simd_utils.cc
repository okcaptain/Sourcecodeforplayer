...
const float kInverseSqrtTwo = 0.707106781186548f;
...
inline size_t GetNumChunks(size_t length) {
...
size_t FindNextAlignedArrayIndex(size_t length, size_t type_size_bytes, size_t memory_alignment_bytes) {
    const size_t byte_length = type_size_bytes * length;
    const size_t unaligned_bytes = byte_length % memory_alignment_bytes;
    const size_t bytes_to_next_aligned = (unaligned_bytes == 0) ? 0 : memory_alignment_bytes - unaligned_bytes;
    return (byte_length + bytes_to_next_aligned) / type_size_bytes;
}

void AddPointwise(size_t length, const float* input_a, const float* input_b, float* output) {
    DCHECK(input_a);
    DCHECK(input_b);
    DCHECK(output);

    const SimdVector* input_a_vector = reinterpret_cast<const SimdVector*>(input_a);
    const SimdVector* input_b_vector = reinterpret_cast<const SimdVector*>(input_b);
    ...
    if (inputs_aligned && output_aligned) {
        ...
        for (size_t i = 0; i < num_chunks; ++i) {
            const SimdVector output_temp = SIMD_ADD(input_a_vector[i], input_b_vector[i]);
    ...
    if (inputs_aligned && output_aligned) {
        ...
        for (size_t i = 0; i < num_chunks * SIMD_LENGTH; ++i) {
...
void SubtractPointwise(size_t length, const float* input_a, const float* input_b, float* output) {
    DCHECK(input_a);
    DCHECK(input_b);
    DCHECK(output);
    ...
    if (inputs_aligned && output_aligned) {
        ...
        for (size_t i = 0; i < num_chunks; ++i) {
            const SimdVector output_temp = SIMD_SUB(input_b_vector[i], input_a_vector[i]);
            _mm_storeu_ps(&output[i * SIMD_LENGTH], output_temp);
        }
    } else if (output_aligned) {
        for (size_t i = 0; i < num_chunks; ++i) {
...
void MultiplyPointwise(size_t length, const float* input_a, const float* input_b, float* output) {
    DCHECK(input_a);
    DCHECK(input_b);
    DCHECK(output);
    ...
    if (inputs_aligned && output_aligned) {
        ...
        for (size_t i = 0; i < num_chunks; ++i) {
            const SimdVector output_temp = SIMD_MULTIPLY(input_a_vector[i], input_b_vector[i]);
            _mm_storeu_ps(&output[i * SIMD_LENGTH], output_temp);
        }
    } else if (output_aligned) {
        for (size_t i = 0; i < num_chunks; ++i) {
...
void MultiplyAndAccumulatePointwise(size_t length, const float* input_a, const float* input_b, float* accumulator) {
    DCHECK(input_a);
    DCHECK(input_b);
    DCHECK(accumulator);
    ...
    if (inputs_aligned && accumulator_aligned) {
        for (size_t i = 0; i < num_chunks; ++i) {
            accumulator_vector[i] = SIMD_MULTIPLY_ADD(input_a_vector[i], input_b_vector[i], accumulator_vector[i]);
        }
    } else if (inputs_aligned) {
        for (size_t i = 0; i < num_chunks; ++i) {
            SimdVector accumulator_temp = _mm_loadu_ps(&accumulator[i * SIMD_LENGTH]);
            accumulator_temp = SIMD_MULTIPLY_ADD(input_a_vector[i], input_b_vector[i], accumulator_temp);
        ...
        for (size_t i = 0; i < num_chunks; ++i) {
            const SimdVector input_a_temp = _mm_loadu_ps(&input_a[i * SIMD_LENGTH]);
            const SimdVector input_b_temp = _mm_loadu_ps(&input_b[i * SIMD_LENGTH]);
            accumulator_vector[i] = SIMD_MULTIPLY_ADD(input_a_temp, input_b_temp, accumulator_vector[i]);
        }
    } else {
        for (size_t i = 0; i < num_chunks; ++i) {
            const SimdVector input_a_temp = _mm_loadu_ps(&input_a[i * SIMD_LENGTH]);
            const SimdVector input_b_temp = _mm_loadu_ps(&input_b[i * SIMD_LENGTH]);
            SimdVector accumulator_temp = _mm_loadu_ps(&accumulator[i * SIMD_LENGTH]);
            accumulator_temp = SIMD_MULTIPLY_ADD(input_a_temp, input_b_temp, accumulator_temp);
            _mm_storeu_ps(&accumulator[i * SIMD_LENGTH], accumulator_temp);
        }
    }
#else
...
void ScalarMultiply(size_t length, float gain, const float* input, float* output) {
    DCHECK(input);
    DCHECK(output);
    ...
    if (input_aligned && output_aligned) {
        ...
        for (size_t i = 0; i < num_chunks; ++i) {
            const SimdVector output_temp = SIMD_MULTIPLY(gain_vector, input_vector[i]);
            _mm_storeu_ps(&output[i * SIMD_LENGTH], output_temp);
        }
    } else if (output_aligned) {
        for (size_t i = 0; i < num_chunks; ++i) {
...
void ScalarMultiplyAndAccumulate(size_t length, float gain, const float* input, float* accumulator) {
    DCHECK(input);
    DCHECK(accumulator);
    ...
    if (input_aligned && accumulator_aligned) {
        for (size_t i = 0; i < num_chunks; ++i) {
            accumulator_vector[i] = SIMD_MULTIPLY_ADD(gain_vector, input_vector[i], accumulator_vector[i]);
        }
    } else if (input_aligned) {
        for (size_t i = 0; i < num_chunks; ++i) {
            SimdVector accumulator_temp = _mm_loadu_ps(&accumulator[i * SIMD_LENGTH]);
            accumulator_temp = SIMD_MULTIPLY_ADD(gain_vector, input_vector[i], accumulator_temp);
            _mm_storeu_ps(&accumulator[i * SIMD_LENGTH], accumulator_temp);
        }
    } else if (accumulator_aligned) {
        for (size_t i = 0; i < num_chunks; ++i) {
            const SimdVector input_temp = _mm_loadu_ps(&input[i * SIMD_LENGTH]);
            accumulator_vector[i] = SIMD_MULTIPLY_ADD(gain_vector, input_temp, accumulator_vector[i]);
        }
    } else {
        for (size_t i = 0; i < num_chunks; ++i) {
            const SimdVector input_temp = _mm_loadu_ps(&input[i * SIMD_LENGTH]);
            SimdVector accumulator_temp = _mm_loadu_ps(&accumulator[i * SIMD_LENGTH]);
            accumulator_temp = SIMD_MULTIPLY_ADD(gain_vector, input_temp, accumulator_temp);
            _mm_storeu_ps(&accumulator[i * SIMD_LENGTH], accumulator_temp);
        }
    }
#else
...
void ApproxComplexMagnitude(size_t length, const float* input, float* output) {
    ...
    if (input_aligned && output_aligned) {
        for (size_t out_index = 0; out_index < num_chunks; ++out_index) {
            ...
            const SimdVector squared_1 = SIMD_MULTIPLY(input_vector[first_index], input_vector[first_index]);
            const SimdVector squared_2 = SIMD_MULTIPLY(input_vector[second_index], input_vector[second_index]);
            const SimdVector unshuffled_1 = _mm_shuffle_ps(squared_1, squared_2, _MM_SHUFFLE(2, 0, 2, 0));
            const SimdVector unshuffled_2 = _mm_shuffle_ps(squared_1, squared_2, _MM_SHUFFLE(3, 1, 3, 1));
        ...
        for (size_t out_index = 0; out_index < num_chunks; ++out_index) {
            const size_t first_index = out_index * 2;
            const size_t second_index = first_index + 1;
            const SimdVector first_temp = _mm_loadu_ps(&input[first_index * SIMD_LENGTH]);
            const SimdVector second_temp = _mm_loadu_ps(&input[second_index * SIMD_LENGTH]);
    ...
    if (input_aligned && output_aligned) {
        ...
        for (size_t out_index = 0; out_index < num_chunks; ++out_index) {
            const size_t first_index = out_index * 2;
            const size_t second_index = first_index + 1;
            const SimdVector first_temp = vld1q_f32(&input[first_index * SIMD_LENGTH]);
            const SimdVector second_temp = vld1q_f32(&input[second_index * SIMD_LENGTH]);
            const SimdVector squared_1 = SIMD_MULTIPLY(first_temp, first_temp);
            const SimdVector squared_2 = SIMD_MULTIPLY(second_temp, second_temp);
            const float32x4x2_t unshuffled = vuzpq_f32(squared_1, squared_2);
            output_vector[out_index] = SIMD_ADD(unshuffled.val[0], unshuffled.val[1]);
    ...
    for (size_t i = length - leftover_samples; i < length; ++i) {
        ...
        const float squared_sum = (input[real_index] * input[real_index]) + (input[imag_index] * input[imag_index]);
        output[i] = 1.0f / FastReciprocalSqrt(squared_sum);
    }
}

void ComplexInterleavedFormatFromMagnitudeAndSinCosPhase(size_t length,
                                                         const float* magnitude,
                                                         const float* cos_phase,
                                                         const float* sin_phase,
                                                         float* complex_interleaved_format_output) {
    size_t leftover_samples = 0;
#ifdef SIMD_NEON
    if (IsAligned(complex_interleaved_format_output) && IsAligned(cos_phase) && IsAligned(sin_phase) &&
        IsAligned(magnitude)) {
        ...
        const SimdVector* magnitude_vec = reinterpret_cast<const SimdVector*>(magnitude);

        const size_t num_chunks = GetNumChunks(length);
        float32x4x2_t interleaved_pair;

        SimdVector* interleaved_vec = reinterpret_cast<SimdVector*>(complex_interleaved_format_output);
        for (size_t i = 0, j = 0; j < num_chunks; ++i, j += 2) {
            interleaved_pair = vzipq_f32(cos_vec[i], sin_vec[i]);
            interleaved_vec[j] = SIMD_MULTIPLY(interleaved_pair.val[0], magnitude_vec[i]);
            interleaved_vec[j + 1] = SIMD_MULTIPLY(interleaved_pair.val[1], magnitude_vec[i]);
        }
    ...
    for (size_t i = leftover_samples, j = leftover_samples / 2; i < length; i += 2, ++j) {
        const size_t imaginary_offset = i + 1;
        complex_interleaved_format_output[i] = magnitude[j] * cos_phase[j];
        complex_interleaved_format_output[imaginary_offset] = magnitude[j] * sin_phase[j];
    }
}

void StereoFromMonoSimd(size_t length, const float* mono, float* left, float* right) {
    ScalarMultiply(length, kInverseSqrtTwo, mono, left);
    std::copy_n(left, length, right);
}

void MonoFromStereoSimd(size_t length, const float* left, const float* right, float* mono) {
    DCHECK(left);
    DCHECK(right);
    DCHECK(mono);
    ...
    if (inputs_aligned && mono_aligned) {
        for (size_t i = 0; i < num_chunks; ++i) {
            mono_vector[i] = SIMD_MULTIPLY(inv_root_two_vec, SIMD_ADD(left_vector[i], right_vector[i]));
        }
    } else if (inputs_aligned) {
        for (size_t i = 0; i < num_chunks; ++i) {
            const SimdVector mono_temp = SIMD_MULTIPLY(inv_root_two_vec, SIMD_ADD(left_vector[i], right_vector[i]));
        ...
        for (size_t i = 0; i < num_chunks; ++i) {
            ...
            mono_vector[i] = SIMD_MULTIPLY(inv_root_two_vec, SIMD_ADD(left_temp, right_temp));
        }
    } else {
        for (size_t i = 0; i < num_chunks; ++i) {
            const SimdVector left_temp = _mm_loadu_ps(&left[i * SIMD_LENGTH]);
            const SimdVector right_temp = _mm_loadu_ps(&right[i * SIMD_LENGTH]);
            const SimdVector mono_temp = SIMD_MULTIPLY(inv_root_two_vec, SIMD_ADD(left_temp, right_temp));
            _mm_storeu_ps(&mono[i * SIMD_LENGTH], mono_temp);
        }
    }
#else
    ...
    if (input_aligned || output_aligned) {
        ...
        if (input_aligned && output_aligned) {
            ...
            for (size_t i = 0; i < GetNumChunks(length); ++i) {
                ...
                temporary_float_vector = SIMD_MULTIPLY(scaling_vector, temporary_float_vector);
...
void InterleaveStereo(size_t length, const int16_t* channel_0, const int16_t* channel_1, int16_t* interleaved_buffer) {
    ...
    size_t leftover_samples = length;
#ifdef SIMD_NEON
    if (IsAligned(interleaved_buffer) && IsAligned(channel_0) && IsAligned(channel_1)) {
        const int16x8_t* channel_0_vec = reinterpret_cast<const int16x8_t*>(channel_0);
        const int16x8_t* channel_1_vec = reinterpret_cast<const int16x8_t*>(channel_1);

        const size_t num_chunks = length / kSixteenBitSimdLength;
        int16x8x2_t interleaved_pair;

        int16x8_t* interleaved_vec = reinterpret_cast<int16x8_t*>(interleaved_buffer);
...
void InterleaveStereo(size_t length, const float* channel_0, const float* channel_1, float* interleaved_buffer) {
    DCHECK(interleaved_buffer);
    DCHECK(channel_0);
    DCHECK(channel_1);
...
#ifdef SIMD_NEON
    if (IsAligned(interleaved_buffer) && IsAligned(channel_0) && IsAligned(channel_1)) {
        const SimdVector* channel_0_vec = reinterpret_cast<const SimdVector*>(channel_0);
        const SimdVector* channel_1_vec = reinterpret_cast<const SimdVector*>(channel_1);

        const size_t num_chunks = GetNumChunks(length);
        float32x4x2_t interleaved_pair;

        SimdVector* interleaved_vec = reinterpret_cast<SimdVector*>(interleaved_buffer);
        for (size_t i = 0, j = 0; i < num_chunks; ++i, j += 2) {
...
void InterleaveStereo(size_t length, const float* channel_0, const float* channel_1, int16_t* interleaved_buffer) {
    DCHECK(interleaved_buffer);
    DCHECK(channel_0);
    DCHECK(channel_1);
    ...
    if (IsAligned(interleaved_buffer) && IsAligned(channel_0) && IsAligned(channel_1)) {
        ...
        int16x4_t* interleaved_vec = reinterpret_cast<int16x4_t*>(interleaved_buffer);
        for (size_t i = 0; i < num_chunks; ++i) {
            const size_t interleaved_index = kNumStereoChannels * i;
            interleaved_pair = vzipq_f32(channel_0_vec[i], channel_1_vec[i]);
            interleaved_pair.val[0] = SIMD_MULTIPLY(scaling_vector, interleaved_pair.val[0]);
            ...
            interleaved_pair.val[1] = SIMD_MULTIPLY(scaling_vector, interleaved_pair.val[1]);
            ...
            interleaved_vec[interleaved_index + 1] = vqmovn_s32(temporary_wide_vector);
        }
    ...
    for (size_t i = length - leftover_samples; i < length; ++i) {
        const size_t interleaved_index = kNumStereoChannels * i;
        interleaved_buffer[interleaved_index] =
            static_cast<int16_t>(std::max(kInt16Min, std::min(kInt16Max, kInt16FromFloat * channel_0[i])));
        interleaved_buffer[interleaved_index + 1] =
            static_cast<int16_t>(std::max(kInt16Min, std::min(kInt16Max, kInt16FromFloat * channel_1[i])));
    }
}

void DeinterleaveStereo(size_t length, const int16_t* interleaved_buffer, int16_t* channel_0, int16_t* channel_1) {
    DCHECK(interleaved_buffer);
    DCHECK(channel_0);
    DCHECK(channel_1);
    ...
    if (IsAligned(interleaved_buffer) && IsAligned(channel_0) && IsAligned(channel_1)) {
        ...
        const int16x8_t* interleaved_vec = reinterpret_cast<const int16x8_t*>(interleaved_buffer);
        for (size_t chunk = 0; chunk < num_chunks; ++chunk) {
            const size_t interleaved_index = chunk * kNumStereoChannels;
            deinterleaved_pair = vuzpq_s16(interleaved_vec[interleaved_index], interleaved_vec[interleaved_index + 1]);
...
void DeinterleaveStereo(size_t length, const float* interleaved_buffer, float* channel_0, float* channel_1) {
    DCHECK(interleaved_buffer);
    DCHECK(channel_0);
    DCHECK(channel_1);
    ...
    if (IsAligned(interleaved_buffer) && IsAligned(channel_0) && IsAligned(channel_1)) {
        ...
        const SimdVector* interleaved_vec = reinterpret_cast<const SimdVector*>(interleaved_buffer);
        for (size_t chunk = 0; chunk < num_chunks; ++chunk) {
            const size_t interleaved_index = chunk * kNumStereoChannels;
            deinterleaved_pair = vuzpq_f32(interleaved_vec[interleaved_index], interleaved_vec[interleaved_index + 1]);
            channel_0_vec[chunk] = deinterleaved_pair.val[0];
            channel_1_vec[chunk] = deinterleaved_pair.val[1];
        }
    }
...
void DeinterleaveStereo(size_t length, const int16_t* interleaved_buffer, float* channel_0, float* channel_1) {
    DCHECK(interleaved_buffer);
    DCHECK(channel_0);
    DCHECK(channel_1);
    ...
    if (IsAligned(interleaved_buffer) && IsAligned(channel_0) && IsAligned(channel_1)) {
        ...
        const int16x4_t* interleaved_vec = reinterpret_cast<const int16x4_t*>(interleaved_buffer);
        for (size_t chunk = 0; chunk < num_chunks; ++chunk) {
            const size_t interleaved_index = chunk * kNumStereoChannels;
            deinterleaved_pair = vuzp_s16(interleaved_vec[interleaved_index], interleaved_vec[interleaved_index + 1]);
            temporary_wide = vmovl_s16(deinterleaved_pair.val[0]);
            channel_0_vec[chunk] = vcvtq_f32_s32(temporary_wide);
            channel_0_vec[chunk] = SIMD_MULTIPLY(scaling_vector, channel_0_vec[chunk]);
            temporary_wide = vmovl_s16(deinterleaved_pair.val[1]);
            channel_1_vec[chunk] = vcvtq_f32_s32(temporary_wide);
            channel_1_vec[chunk] = SIMD_MULTIPLY(scaling_vector, channel_1_vec[chunk]);
        }
    }
#endif  // SIMD_NEON
    for (size_t i = length - leftover_samples; i < length; ++i) {
        const size_t interleaved_index = kNumStereoChannels * i;
        channel_0[i] = static_cast<float>(interleaved_buffer[interleaved_index]) * kFloatFromInt16;
        channel_1[i] = static_cast<float>(interleaved_buffer[interleaved_index + 1]) * kFloatFromInt16;
    }
}

void InterleaveQuad(size_t length,
                    const int16_t* channel_0,
                    const int16_t* channel_1,
                    const int16_t* channel_2,
                    const int16_t* channel_3,
                    int16_t* workspace,
    ...
    int16_t* workspace_half_point =
        workspace + FindNextAlignedArrayIndex(double_length, sizeof(int16_t), kMemoryAlignmentBytes);
    InterleaveStereo(length, channel_0, channel_2, workspace);
    InterleaveStereo(length, channel_1, channel_3, workspace_half_point);
    InterleaveStereo(double_length, workspace, workspace_half_point, interleaved_buffer);
#else
    for (size_t i = 0; i < length; ++i) {
...
void InterleaveQuad(size_t length,
                    const float* channel_0,
                    const float* channel_1,
                    const float* channel_2,
                    const float* channel_3,
                    float* workspace,
    ...
    float* workspace_half_point =
        workspace + FindNextAlignedArrayIndex(double_length, sizeof(float), kMemoryAlignmentBytes);
...
void DeinterleaveQuad(size_t length,
                      const int16_t* interleaved_buffer,
                      int16_t* workspace,
                      int16_t* channel_0,
                      int16_t* channel_1,
                      int16_t* channel_2,
    ...
    int16_t* workspace_half_point =
        workspace + FindNextAlignedArrayIndex(double_length, sizeof(int16_t), kMemoryAlignmentBytes);
    DCHECK(IsAligned(workspace_half_point));
    DeinterleaveStereo(double_length, interleaved_buffer, workspace, workspace_half_point);
...
void ScalarChunkMultiplyAndAccumulate(size_t chunk, float gain, const float* input, float* accumulator) {
    const SimdVector* input_vector = reinterpret_cast<const SimdVector*>(input);
    SimdVector* accumulator_vector = reinterpret_cast<SimdVector*>(accumulator);

    const SimdVector gain_vector = SIMD_LOAD_ONE_FLOAT(gain);

    const size_t num_chunks = chunk;
    const bool input_aligned = vraudio_simd::IsAligned(input);
    const bool accumulator_aligned = vraudio_simd::IsAligned(accumulator);
